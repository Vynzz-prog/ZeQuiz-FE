...
import com.example.zequiz.activities.DetailSoalActivity
import com.example.zequiz.adapters.KuisSiswaAdapter
...
class HomeSiswaFragment : Fragment() {
    ...
    private lateinit var kuisAdapter: KuisSiswaAdapter
    ...
    override fun onCreateView(
        ...
        kuisRecyclerView = view.findViewById(R.id.recyclerViewKuisSiswa)
        kuisRecyclerView.layoutManager = LinearLayoutManager(requireContext())
        kuisAdapter = KuisSiswaAdapter(kuisList) { kuis ->
            val intent = Intent(requireContext(), DetailSoalActivity::class.java)
    ...
    private fun loadKuis() {
        ...
        ApiService.instance.getKuisSiswa(token).enqueue(object : Callback<List<KuisResponse>> {
